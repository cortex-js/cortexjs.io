...
/**
 * Epsil source highlighted inline, outside a Docusaurus code block.
 *
 * ` ```epsil ` blocks go through the swizzled `theme/CodeBlock/Content`; this
 * is for the places that render Epsil without that chrome, like the REPL's
 ...
 * tokens Prism itself produces (it has no Epsil grammar). Lines are emitted as
 ...
 */
export default function EpsilCode({
  code,
  className,
}: {
  code: string;
  ...
  const lines = React.useMemo(() => tokenizeEpsil(code), [code]);

  return (
    <Highlight theme={prismTheme} code={code} language="epsil">
      {({ getTokenProps }) => (
