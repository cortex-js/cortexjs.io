// Epsil syntax highlighting for Docusaurus code blocks.
//
// Docusaurus highlights with Prism, but the Epsil language definition is
// written for highlight.js and maintained next to the grammar it describes, in
// the compute-engine repo (`src/epsil/highlight-js-mode.js`, with its keyword
// and constant tables pinned by `test/epsil/reserved-words.test.ts`).
// `scripts/build.sh` syncs that file here as `./epsil-mode.js`.
//
...
import epsilMode from './epsil-mode.js';

hljs.registerLanguage('epsil', epsilMode);

// highlight.js scopes -> Prism token types, so the existing Prism theme
// (`src/prism/theme-light.js`) colors Epsil without a second palette.
//
// `variable` is deliberately mapped to `plain`: the Epsil mode matches *every*
...
/**
 * Tokenize Epsil source into `prism-react-renderer`-shaped lines of tokens.
 *
 * @param {string} code
 * @returns {{types: string[], content: string, empty?: boolean}[][]}
 */
export default function tokenizeEpsil(code) {
